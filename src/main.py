...
def generate_nodes(n, area_length, connection_range, layout='linear'):
    ...
    if layout == 'linear':
        nodes = [Node(f"[node-{i}]", position=(i*connection_range*0.99, area_length//2), connection_range=connection_range, size_km=area_length) for i in range(0, n)]
    else: 
...
def create_simulation(context:'Context', layout='random', node_info=None):
